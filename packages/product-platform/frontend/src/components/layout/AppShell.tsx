    ...
    <CurrentUserProvider user={user}>
      <div className="min-h-screen bg-canvas text-foreground">
          ...
          <div className="px-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs leading-4 text-sidebar-muted">Agent governance</p>
              </div>
            </div>
        ...
        <div className="lg:pl-72">
          ...
          {tenant.isError ? (
            <main className="min-h-[calc(100vh-4rem)] p-6" tabIndex={-1}>
              <ErrorState message={tenant.error?.message ?? "Unable to load tenant context."} />
            </main>
          ) : tenant.isLoading || !tenant.isReady ? (
            <main className="min-h-[calc(100vh-4rem)]" tabIndex={-1}>
              <LoadingState label="Loading tenant environment" />
            </main>
          ) : (
            <TenantQueryScopeProvider context={tenant.tenantContext}>
