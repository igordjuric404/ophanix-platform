...
expects gateway contract `tool-gateway.v1`, enforces the advertised
`min_sdk_version`, and reads gateway-published payload/response limits,
pagination modes, idempotency TTLs, rate limits, and circuit-breaker settings.
Set `require_compatible_gateway=True` to make clients fail closed before
runtime operations if that probe is incompatible. `get_tool()` reports
