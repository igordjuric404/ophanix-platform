...
This directory is the Product Platform internal compatibility package named
`ophanix-tool-gateway-sdk`. It exists so Product Platform CI and release
validation can exercise the same `ophanix_tool_gateway` import path before
external publishing. The canonical public Python distribution is
`ophanix-python-sdk`, which also exposes `ophanix_tool_gateway`.
...
For unreleased internal Product Platform validation, install this compatibility
package from a wheel generated by `python3 scripts/validate_release.py`, or
